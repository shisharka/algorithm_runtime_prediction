...
class GCRF:
    ...
    def muKov(alfa,R,Precison,Noinst,NodeNo):
        ...
        # Kov = np.linalg.inv(Precison) - ne radi kad je Precision matrica singularna
        Kov = np.linalg.pinv(Precison)
...
# FIT - LEARN    
    def fit(self,R,Se,y,x0 = None,learn = 'SLSQP', maxiter = 50, learnrate = 0.01):
        ...
        Noinst = Se.shape[0]
        NoGraph = Se.shape[1]
        # cons = ({'type':'ineq', 'fun': lambda x: x}) - ovo nije radilo
        cons = []
        bnds = []
        for i in range(0, ModelUNNo + NoGraph):
            cons.append({'type': 'ineq', 'fun': lambda x: x[i]}) # nenegativne koordinate vektora x
            bnds.append((0, None)) # da bi koordinate vektora x bile strogo pozitivne
        cons = tuple(cons)
        bnds = tuple(bnds)
        if x0 == None:
            x0 = np.abs(np.random.randn(ModelUNNo + NoGraph))*100

        if learn == 'SLSQP':
            res = minimize(L, x0, method='SLSQP', jac=dLdX, args=(ModelUNNo,NoGraph,NodeNo,Noinst,R,Se,y), options={'disp': True,'maxiter':300},constraints=cons, bounds=bnds)
...
mod1.fit(R,Se,vrednosti, learn = 'SLSQP')
...
print('Acc score je {} '.format(mean_squared_error(vrednosti.reshape(broj),vrednosti1.reshape(broj))))
print('Acc score je {} '.format(mean_squared_error(vrednosti.reshape(broj),R[:,0])))
print('Acc score je {} '.format(mean_squared_error(vrednosti.reshape(broj),R[:,1])))